...
f"- **Forgejo** at `https://git.cove.local/` (TLS via cove certs). "
...
f"Use `gh pr ready` / `gh pr ready --undo` to toggle draft status.\n"
        f"- **Vault** at `https://vault.cove.local/` (TLS via cove certs). "
        f"Use `cove creds vault-get` / `cove creds vault-put` — never hardcode secrets.\n"
        f"- **Prerequisites (macOS):** `brew install colima && colima start`\n"
        f"- **Constraints:** No cloud dependencies. No internet during builds/CI. "
        f"Cove's own git remotes go to Forgejo at `git.cove.local`.\n"
        f"- **New projects / Git remotes:** Use `origin` for your primary remote "
        f"(e.g., GitHub, GitLab, or Cove itself). "
        f"Add a secondary remote named after the forge: `fj` for Cove/Forgejo, "
        f"`gh` for GitHub, `gl` for GitLab.\n\n"
...
def _write_project_override(project_root: Path, ctx: dict) -> Path:
    ...
    content = (
        ...
        f"- Use `origin` for your primary remote, `fj` for Cove/Forgejo.\n\n"
        "## Local dev workflow\n\n"
